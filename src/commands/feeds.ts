import {Feed, User} from "../lib/db/schema.ts";
import {createFeed, getFeeds} from "../lib/db/queries/feeds.ts";
import {getUserById} from "../lib/db/queries/users.ts";
import {createFeedFollow} from "../lib/db/queries/feedFollows.ts";

export async function handlerAddFeed(cmdName: string, user: User, ...args: string[]): Promise<void> {

    if (args.length !== 2) {
        throw new Error(`The ${cmdName} command should have two arguments.`);
    }

    const feedName = args[0], url = args[1];

    const feed = await createFeed(feedName, url, user.id);
    if (!feed) {
        throw new Error(`Failed to create feed`);
    }

    console.log("Feed created successfully:");

    await createFeedFollow(user.id, feed.id)

    printFeed(feed, user);
}

export async function handlerFeeds(cmdName: string, ...args: string[]): Promise<void> {

    if (args.length > 0) {
        throw new Error(`The ${cmdName} command shouldn't have arguments.`);
    }

    const feeds: Feed[] = await getFeeds();

    if (feeds.length === 0) {
        console.log(`No feeds found.`);
        return;
    }

    console.log(`Found %d feeds:\n`, feeds.length);
    for (const feed: Feed of feeds) {
        const user: User = await getUserById(feed.userId);
        if (!user) {
            throw new Error(`Failed to find user for feed ${feed.id}`);
        }

        printFeed(feed, user);
        console.log(`=====================================`);
    }
}

function printFeed(feed: Feed, user: User) {
    console.log(`* ID:            ${feed.id}`);
    console.log(`* Created:       ${feed.createdAt}`);
    console.log(`* Updated:       ${feed.updatedAt}`);
    console.log(`* name:          ${feed.name}`);
    console.log(`* URL:           ${feed.url}`);
    console.log(`* User:          ${user.name}`);
}